...
![기능 요구사항](images/requirements.png)
...
## 기술 방향 전환 (가우시안 스플래팅 → SMPL-X)

초기에는 가우시안 스플래팅으로 실제 외형을 복원하려 했으나, 배경 특징점·마스크·카메라 포즈 문제로 형상이 안정적으로 수렴하지 못하고 흐릿한 덩어리 형태로 나타났습니다. 신체 치수 측정과 의류 정합이라는 목적에는 **일정한 위상·관절 구조를 갖는 SMPL-X 파라미터형 인체 모델**이 더 적합하다고 판단해, 최종 시스템의 핵심 기술을 SMPL-X 기반으로 전환했습니다.
...
* **메쉬 정확도** — 단일 영상 기반 근사 메쉬 → 다중 시점 SMPL-X 파라미터 최적화로 정밀도 향상
* **의류 정합 품질** — 의류 3D 모델 품질에 의존 → 모델 표준화·정렬 기준 고도화
* **의류 질감** — UV·텍스처 부재로 단색까지 표현 → UV·텍스처 포함 모델 확보
